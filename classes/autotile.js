...
class autotile {
  ...
  this.x_boundary = this.width - 1;
 this.y_boundary = this.height - 1;

  for (let y = 0; y < this.height; y++) {
    this.tiles[y] = [];
    for (let x = 0; x < this.width; x++) {
      this.tiles[y][x] = this.autotileLookup(map, this.x_boundary, this.y_boundary, x, y);
    }
  }

  return this.tiles;
  }
  autotileLookup(map, x_boundary, y_boundary, x, y) {
  let sum = 0;
  let n = false;
  let e = false;
  let s = false;
